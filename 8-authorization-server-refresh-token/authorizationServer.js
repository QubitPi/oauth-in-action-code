...
app.post("/token", function(request, response){
    ...
    if (request.body.grant_type == 'authorization_code') {
        ...
        nosql.one().make(function(builder) {
            builder.where("refresh_token", request.body.refresh_token);
            builder.callback(function(error, token) {
                if (token) {
                    console.log("We found a matching refresh token: %s", request.body.refresh_token);

                    if (token.client_id != clientId) {
                        nosql.remove().make(function(builder) {
                            builder.where("refresh_token", request.body.refresh_token);
                        })
                        response.status(400).json({error: "invalid grant"});
                        return;
                    }
                    ...
                    var message = "No matching refresh token was found.";
                    console.log(message);
