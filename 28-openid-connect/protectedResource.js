...
var userInfoEndpoint = function(request, response) {
    if (!__.contains(request.access_token.scope, "openid")) {
        response.status(403).end();
        return;
    }

    var user = request.access_token.user;
    if (!user) {
    ...
    var out = {};
    __.each(request.access_token.scope, function(scope) {
        if (scope == "openid") {
